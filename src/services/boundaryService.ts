...
// Fetch live administrative boundary GeoJSON from Nominatim OSM API for any country or admin area worldwide
export async function fetchLiveBoundaryGeoJSON(query: string, levelHint?: string): Promise<AdminBoundaryItem[]> {
  try {
    const clean = query.trim();
    if (!clean) return [];

    // If searching specifically for a country or general global search, don't restrict to vn
    const isCountrySearch = levelHint === 'country';
    const encoded = encodeURIComponent(clean);
    
    // First attempt: global search or country search
    let url = isCountrySearch
      ? `https://nominatim.openstreetmap.org/search?q=${encoded}&format=geojson&polygon_geojson=1&featuretype=country&addressdetails=1&limit=8`
      : `https://nominatim.openstreetmap.org/search?q=${encoded}&format=geojson&polygon_geojson=1&addressdetails=1&limit=8`;
    ...
    if (!res.ok && !isCountrySearch) {
      // Fallback with Vietnam context if not country
      url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(`${clean}, Vietnam`)}&format=geojson&polygon_geojson=1&countrycodes=vn&addressdetails=1&limit=8`;
    ...
    const items: AdminBoundaryItem[] = data.features
      ...
      .map((f: any, idx: number) => {
        ...
        const displayName = props.display_name || props.name || clean;
        const name = props.name || clean;
        const addresstype = props.addresstype || props.type || '';
        const osmType = props.osm_type || '';

        let level: 'country' | 'province' | 'district' | 'ward' = 'ward';
        let levelLabel = 'Khu vực';
        let zoom = 14.5;

        if (addresstype === 'country' || props.country === name || isCountrySearch || (props.extratags?.admin_level === '2')) {
          level = 'country';
          levelLabel = 'Quốc gia 🌍';
          zoom = 4.5;
        } else if (addresstype === 'state' || addresstype === 'province' || addresstype === 'city' || props.extratags?.admin_level === '4') {
          level = 'province';
          levelLabel = 'Tỉnh / Thành phố';
          zoom = 9.3;
        } else if (addresstype === 'county' || addresstype === 'district' || addresstype === 'town' || ['5', '6'].includes(props.extratags?.admin_level || '')) {
          level = 'district';
          levelLabel = 'Quận / Huyện';
          zoom = 12.0;
        } else if (addresstype === 'suburb' || addresstype === 'quarter' || addresstype === 'village' || ['7', '8', '9', '10'].includes(props.extratags?.admin_level || '')) {
          level = 'ward';
